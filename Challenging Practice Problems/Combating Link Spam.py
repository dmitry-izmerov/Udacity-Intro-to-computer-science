...
def compute_ranks(graph, k):
    ...
    for i in range(0, numloops):
        ...
        for page in graph:
            newrank = (1 - d) / npages
            for node in graph:
                if page in graph[node] and not is_reciprocal_link(graph, node, page, k):
...
def is_reciprocal_link(graph, start, end, k):
    if k == 0:
        if start == end:
            return True
        else:
    ...
    if start in graph[end]:
        return True
    for page in graph[end]:
        if is_reciprocal_link(graph, start, page, k - 1):
            return True
    return False
