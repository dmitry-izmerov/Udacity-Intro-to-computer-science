...
def create_data_structure(string_input):
    ...
    if len(string_input) > 0:
        ...
        for i in range(0, len(strings) - 1, 2):
            friends_string = strings[i]
            games_string = strings[i+1]

            # parse friends
            connected = friends_string.split('is connected to')
            user = connected[0].strip()
            friends = [i.strip() for i in re.split(r'\s*,\s*', connected[1])]
            network[user] = {'friends' : friends}

            # parse games
            liked = games_string.split('likes to play')
            user = liked[0].strip()
            games = [i.strip() for i in re.split(r'\s*,\s*', liked[1])]
...
def add_connection(network, user_A, user_B):
    if user_A in network and user_B in network:
        if 'friends' in network[user_A]:
            if not user_B in network[user_A]['friends']:
                network[user_A]['friends'].append(user_B)
        else:
            network[user_A]['friends']= [user_B]
        return network
    else:
        return False
    ...
    if user in network:
        if 'friends' in network[user]:
            ...
            for friend in network[user]['friends']:
                if 'friends' in network[friend]:
                    for connection in network[friend]['friends']:
                        if not connection in secondary_connections:
...
def connections_in_common(network, user_A, user_B):
    if user_A in network and user_B in network:
        commons = []
        for user_A_friend in network[user_A]['friends']:
            if user_A_friend in network[user_B]['friends']:
                commons.append(user_A_friend)
        return len(commons)
    return False
...
def path_to_friend(network, user_A, user_B):
    user_last = user_A.split('=')[-1]
    if user_last in network and user_B in network:
        if user_B in network[user_last]['friends']:
            return [user_last, user_B]
        else:
            prev_steps = user_A.split('=')
            for friend in network[user_last]['friends']:
                if not friend in prev_steps:
                    res = path_to_friend(network, user_A + "=" + friend, user_B)
                    if res:
                        return [user_last] + res
...
network = add_new_user(network,'Frank',[])
network = add_new_user(network,'Gretel',[])
network = add_connection(network,'Eve','Frank')
network = add_connection(network,'Eve','Gretel')
print(path_to_friend(network, 'Gretel', 'Alice'))
